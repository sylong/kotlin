...
public final class CallExpressionTranslator extends AbstractCallExpressionTranslator {
    ...
    @Nullable
    private VariableDescriptor getVariableByName(@NotNull LexicalScope scope, @NotNull Name name) {
        LookupLocation lookupLocation = new KotlinLookupLocation(resolvedCall.getCall().getCallElement());
        while (true) {
            Collection<VariableDescriptor> variables = scope.getContributedVariables(name, lookupLocation);
